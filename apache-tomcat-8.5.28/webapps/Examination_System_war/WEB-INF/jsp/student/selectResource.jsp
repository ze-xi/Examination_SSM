            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    <div class="row">
                        ...
                        <form class="col-md-2 inline-block" action="/student/upload" method="post"
                ...
                <table id="MyTable" class="table-hover table table-bordered table-condensed" style="overflow-y: hidden">
                    <thead>
                    <tr>
                        <th>编号</th>
                        <th>职称</th>
                ...
                <div class="panel-footer">
                    <c:if test="${pagingVO != null}">
                        <nav style="text-align: center">
                            <ul class="pagination">
                                <li><a href="/student/showResource?page=${pagingVO.upPageNo}">&laquo;上一页</a></li>
                                <li class="active"><a href="">${pagingVO.curentPageNo}</a></li>
                                <c:if test="${pagingVO.curentPageNo+1 <= pagingVO.totalCount}">
                                    <li>
                                        <a href="/student/showResource?page=${pagingVO.curentPageNo+1}">${pagingVO.curentPageNo+1}</a>
                                    </li>
                                </c:if>
                                <c:if test="${pagingVO.curentPageNo+2 <= pagingVO.totalCount}">
                                    <li>
                                        <a href="/student/showResource?page=${pagingVO.curentPageNo+2}">${pagingVO.curentPageNo+2}</a>
                                    </li>
                                </c:if>
                                <c:if test="${pagingVO.curentPageNo+3 <= pagingVO.totalCount}">
                                    <li>
                                        <a href="/student/showResource?page=${pagingVO.curentPageNo+3}">${pagingVO.curentPageNo+3}</a>
                                    </li>
                                </c:if>
                                <c:if test="${pagingVO.curentPageNo+4 <= pagingVO.totalCount}">
                                    <li>
                                        <a href="/student/showResource?page=${pagingVO.curentPageNo+4}">${pagingVO.curentPageNo+4}</a>
                                    </li>
                                </c:if>
                                <li><a href="/student/showResource?page=${pagingVO.totalCount}">最后一页&raquo;</a></li>
                            </ul>
                        </nav>
                    </c:if>
                </div>
...
// 获取特定参数的值
    ...
    function onceDownload(row,id) {
        if(id !== ""){
            $.ajax({
                url: "/student/onceDownload?id="+id,
                method: "GET",
                xhrFields: {
                    responseType: 'blob'
                },
    ...
    function deleteFunction(row,id,courseId) {
        console.log("删除中");
        <%--location.href='/student/removeResource?id=${item.id}&CourseId=${item.courseId}'--%>
        var getMethod="/student/removeResource?id="+id+"&CourseId="+courseId;
